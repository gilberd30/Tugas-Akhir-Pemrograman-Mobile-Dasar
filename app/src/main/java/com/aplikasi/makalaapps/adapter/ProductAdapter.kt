...
class ProductAdapter(var context: Context, var list:ArrayList<Product>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val view:View = LayoutInflater.from(context).inflate(R.layout.adapter_beranda, parent,
                false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        (holder as myProductAdapter).adapter(list[position].nama, list[position].harga,
                list[position].lokasi,
