    ...
    private fun loginUser(){
        if(validateLoginDetails()){
            ...
            val stringRequest = StringRequest(
                    Request.Method.GET, url+"?email="+et_email.text.toString()+
                    ...
                    { response ->
                        if (response == "1"){
                            ...
                            hideProgressDialog()
                            showErrorSnackBar(resources.getString(R.string.account_not_found),
                                    true)
                        }
                    },
                    { error ->
        ...
        val reques = JsonArrayRequest(Request.Method.GET,
                ...
                { response ->
                    for (s in 0..response.length() - 1) {
                        ...
                        GlobalData.potoProfil = job.getString("poto_profil").replace(
                                "localhost", "192.168.100.17")
                    }
                },
                { error ->
